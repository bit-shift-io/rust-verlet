...
impl Body {
    ...
    pub fn create_stick_spoke_wheel(origin: Vector2<f32>) -> Self {
        ...
        for i in 0..half_divisions { 
            ...
            let stick = {
                let p1: Rc<RefCell<dyn Position>> = body.particles[i].clone();
                let p2: Rc<RefCell<dyn Position>> = body.particles[opposite_division].clone();
        ...
        for i in 0..divisions {
            let p1 : Rc<RefCell<dyn Position>> = body.particles[i].clone();
            let p2 : Rc<RefCell<dyn Position>> = if (i + 1) == divisions { body.particles[0].clone() } else { body.particles[i + 1].clone() };
